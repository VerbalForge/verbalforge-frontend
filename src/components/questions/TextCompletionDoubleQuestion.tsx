...
export function TextCompletionDoubleQuestion({ question, onNext, onPrev, hasNext = false, hasPrev = false }: TextCompletionDoubleQuestionProps) {
  ...
  const handleSubmit = () => {
    if (selectedBlank1 === null || selectedBlank2 === null) return;
    setIsSubmitted(true);
  };
  ...
  const getOptionStatus = (shuffledIndex: number, isBlank1: boolean) => {
    const shuffledOptions = isBlank1 ? shuffledBlank1 : shuffledBlank2;
    const selectedIndex = isBlank1 ? selectedBlank1 : selectedBlank2;
  ...
  const getResultMessage = () => {
    ...
    if (blank1Correct && blank2Correct) {
      ...
      return {
        type: 'partial',
        message: 'Partially correct. One answer is correct.',
      };
    } else {
      return {
        type: 'error',
  ...
  const renderOption = (
    ...
    const status = getOptionStatus(shuffledIndex, isBlank1);
    const isCorrect = status.choice?.is_correct || false;
    const isSelected = status.isSelected;

    return (
      <div
        key={`blank${isBlank1 ? 1 : 2}-${shuffledIndex}`}
        ...
        onClick={() => (isBlank1 ? handleBlank1Toggle(shuffledIndex) : handleBlank2Toggle(shuffledIndex))}
      >
      ...
      <div className="flex-1 overflow-y-auto space-y-2.5 pr-2">
        ...
        <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
            {/* Row 1 */}
            {renderOption(shuffledBlank1, 0, true)}
            {renderOption(shuffledBlank2, 0, false)}
            
            {/* Row 2 */}
            {renderOption(shuffledBlank1, 1, true)}
            {renderOption(shuffledBlank2, 1, false)}
            
            {/* Row 3 */}
            {renderOption(shuffledBlank1, 2, true)}
            {renderOption(shuffledBlank2, 2, false)}
          </div>
        </div>
      </div>
